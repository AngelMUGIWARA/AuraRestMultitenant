...
export function AdminTopbar() {
  ...
  return (
    <header className="sticky top-0 z-10 flex h-[60px] items-center gap-2 border-b border-maison-border bg-surface-1 px-4 transition-colors lg:px-5">
      ...
      <span className="text-sm font-medium text-maison-cream sm:hidden">{crumbs.at(-1)?.label ?? 'Admin'}</span>
      ...
      <div className="relative mx-3 hidden max-w-xs flex-1 lg:flex">
        ...
        <input id="topbar-search" type="search" placeholder="Buscar..."
          className={cn('h-8 w-full rounded border border-maison-border bg-surface-2 pl-8 pr-12 text-sm text-maison-cream placeholder:text-maison-cream-dim outline-none transition-colors focus:border-maison-amber')} />
