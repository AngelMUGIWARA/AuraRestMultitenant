...
export function useUsers(branchId: string) {
  ...
  const setFilters = useCallback((patch: Partial<UserFilters>) => setFiltersState((p) => ({ ...p, ...patch, page: 1 })), []);
  useEffect(() => {
    let cancelled = false;
    setIsLoading(true); setError(null);
    const branchFilter = branchId !== 'global' ? branchId : undefined;
    Promise.all([usersService.getStats(branchId), usersService.getAll({ ...filters, branchId: branchFilter })])
      .then(([s, u]) => { if (!cancelled) { setStats(s.data); setUsers(u); } })
