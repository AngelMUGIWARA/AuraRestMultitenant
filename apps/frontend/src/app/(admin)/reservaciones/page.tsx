...
import {
  IconCalendar, IconUsers, IconClock, IconPlus, IconRefresh,
  IconAlertCircle, IconSearch,
...
function ReservationRowSkeleton() {
  return (
    <div className="flex items-center gap-3 border-b border-maison-border px-5 py-4 last:border-b-0">
      <Skeleton className="h-7 w-14 rounded flex-shrink-0" /> {/* time */}
      <SkeletonAvatar size="sm" />
      <div className="min-w-0 flex-1 space-y-1.5">
        <Skeleton className="h-3 w-32" />
        <Skeleton className="h-2.5 w-24" />
      </div>
      <Skeleton className="h-3 w-10 flex-shrink-0" /> {/* party size */}
      <Skeleton className="h-3 w-14 flex-shrink-0" /> {/* table */}
      <Skeleton className="h-5 w-20 flex-shrink-0 rounded-full" /> {/* status */}
    </div>
  );
}
...
function ReservationRow({
  confirmationCode, guestName, guestPhone, time, partySize, tableName, status, notes,
}: ReservationRowProps) {
  return (
...
export default function ReservacionesPage() {
  ...
  return (
    <div className="flex flex-col gap-7 animate-fade-in">
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1.5 text-sm capitalize text-maison-cream-muted">
            {today}
            {!selectedBranch.isGlobal && ` — ${selectedBranch.name}`}
          </p>
        </div>
      ...
      {/* ── Métricas Primero ───────────────────────────────────
           Regla obligatoria: KPIs siempre antes de listas o tablas. */}
          ...
          <section className="card overflow-hidden" aria-labelledby="res-list-title">
            ...
            {isLoading && (
              <div aria-hidden="true">
                {Array.from({ length: 7 }).map((_, i) => <ReservationRowSkeleton key={i} />)}
              </div>
            )}
